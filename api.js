...
export async function fetchWeather(city, apiKey = null) {
  ...
  // Clean API key string (handles localStorage stringified null/undefined values)
  const cleanKey = apiKey && apiKey.trim() !== '' && apiKey !== 'null' && apiKey !== 'undefined' ? apiKey.trim() : null;
  ...
  const currentUrl = `https://api.openweathermap.org/data/2.5/weather?q=${encodedCity}&appid=${cleanKey}&units=metric`;
  const forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${encodedCity}&appid=${cleanKey}&units=metric`;
